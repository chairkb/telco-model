...
public class MyanmarPassport extends Identity implements Passport {
	@ReflectField(type=FieldType.STRING, size=32, indexes={@ReflectIndex(indexName="idx_mynm_paspt", indexNo=0, indexOrder=SortOrder.ASC, isUnique=true)}, displayPosition=300, updateable=false) public static String PassportNo;
	
	@Override
	public void initBeforePopulate() throws Exception {
		this.setIssuingCountry("Myanmar");
	...
	public void validateBeforePersist(Connection aConn) throws Exception {
		...
		if (this.getPassportNo() != null) {
			if (!this.getPassportNo().trim().isEmpty()) {
				...
				throw new Hinderance("Mynamar passport number cannot be empty");
			}
		}
	}
}
