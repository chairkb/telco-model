...
public class ResultSetFetch{
	...
	public void forEachFetch(Connection aConn, Class aClass, String aSqlToGetObjId, List<Field> aPositionalParamValue, ResultSetFetchIntf aCallback) throws Exception {
		this.conn = aConn;
		this.fetchClass = aClass;
		String pkColName = Clasz.CreatePkName(this.fetchClass);
		...
		try {
			while(this.rs.next()) {
				...
				aCallback.callback(aConn, clasz);
			}
		} catch (Exception ex) {
