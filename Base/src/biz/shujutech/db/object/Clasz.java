...
public class Clasz extends Table implements Comparable {
	...
	*/
	...
	public static String getMemberOfPrefix() {
	...
	public String createSequenceName() {
		String result = getSequenceNamePrefix() + Database.Java2DbTableName(this.getClass().getSimpleName());
		return(result);
	}

	public static String CreatePkName(Class aClass) throws Exception {
	...
	public String getChildCountNameFull(Connection aConn) throws Exception {
		...
		if (Database.GetDbType(aConn) == DbType.MYSQL || Database.GetDbType(aConn) == DbType.ORACLE) {
			result = this.getTableName() + "." + this.getChildCountName();
		} else {
			result = this.getChildCountName();
		}
		return(result);
	}

	public String getChildCountName() throws Exception {
		String result;
	...
	public void createFieldChildCount(Record aRec) throws Exception {
		FieldInt countField = (FieldInt) aRec.getField(this.getChildCountName());
		if (countField == null) {
			countField = (FieldInt) aRec.createField(this.getChildCountName(), FieldType.INTEGER);
	...
	public String getInheritanceTableName() {
		String result = GetInheritanceTableName(this.getClass());
		return(result);
	}

	public static String GetInheritanceTableName(Class aChild) {
		String ihName = Clasz.getInheritancePrefix() + Database.Java2DbTableName(aChild.getSimpleName()); // create the parent ih_ table
		return(ihName);
	}
	...
	*/
	...
	public String getMemberOfTableName() {
		String result = GetMemberOfTableName(this.getClass());
		return(result);
	}

	public static String GetMemberOfTableName(Class aChild) {
		String ivName = Clasz.getMemberOfPrefix() + Database.Java2DbTableName(aChild.getSimpleName()); // create the parent iv_ table
		return(ivName);
	}

	public String getBoxMemberTableName(String aFieldName) {
		String result = GetBoxMemberTableName(this.getClass(), aFieldName);
		return(result);
	}

	public static String GetBoxMemberTableName(Class aParent, String aFieldName) {
		String ivName = Clasz.getMemberOfPrefix() + Database.Java2DbTableName(aParent.getSimpleName()) + "_" + Database.Java2DbTableName(aFieldName); // create the parent iv_ table
		return(ivName);
	}
	...
	public void setValueInt(String aFieldName, int aFieldValue) throws Exception {
		if (this.fieldExist(aFieldName)) {
	...
	public long getNextObjectId(Connection aConn) throws Exception {
		long result = Database.GetNextSequence(aConn, this.createSequenceName());
		return(result);
	}
	...
	public void update(Connection aConn) throws Exception {
		...
		recWhere.createField(rec2Update.getField(rec2Update.getPkName()));
		recWhere.copyValue(rec2Update.getField(rec2Update.getPkName()));
	...
	public Clasz getChildObject(Connection aConn, Clasz aLeafObject) throws Exception {
		...
		if (childObject != null) {
			String ihName = childObject.getInheritanceTableName();

			Record recSelect = new Record();
			recSelect.createField(Clasz.CreatePkName(childObject.getClass()), FieldType.LONG); // select the primary key of the child clszObject
	...
	public void ForEachClasz(Connection aConn, Record aWhere, ResultSetFetchIntf  aCallback) throws Exception {
		ForEachClasz(aConn, this.getClass(), aWhere, aCallback);
	}

	public static void ForEachClasz(Connection aConn, Class aClaszClass, Record aWhere, ResultSetFetchIntf  aCallback) throws Exception {
		PreparedStatement stmt = null;
		ResultSet rs = null;
		try {
			...
			while (rs.next()) {
				...
				clasz.populateObject(aConn, rs, false);
				if (aCallback != null) {
					if (aCallback.callback(aConn, clasz) == false) {
						break;
					}
				}
			}
	...
	*/
	...
	private static Clasz CreateObjectDdl(Connection aConn, Class aClass, boolean aDoDdl, List aAvoidRecursive, int aDepth, boolean aInline) throws Exception {
		...
		if (aDoDdl && Database.TableExist(aConn, clszObject.getTableName()) == false) {
			...
			Database.CreateSequence(aConn, clszObject.createSequenceName()); // alter the table to make the primary key auto increment
					...
					if (fieldType == FieldType.OBJECT) {
						...
						if (fieldCreated && aDepth > RECURSIVE_DEPTH) {
							...
							if (FieldObject.IsAbstract(eachAnnotation.clasz())) {
								...
								metaObj = Clasz.CreateObject(odb, aConn, memberClass, null, true, aAvoidRecursive, aDepth + 1, false);
							}
						}
						...
						ObjectBase.createBoxMember(aConn, aParent.getClass(), dbFieldName); // now create the table to associate the array of "member of" relationship
					} else {
	...
	private static Clasz fetchObjectSlow(Connection aConn, Clasz aCriteria) throws Exception {
		...
		if (aCriteria.getObjectId() == null || aCriteria.isPopulated() == false) { // if no objectId, we do fetch for it using given criteria in the object fields
			objectWithPk = FetchObjectForPk(aConn, aCriteria);
		} else {
	 ...
	 * @throws Exception
	 */
	public static Clasz FetchObjectForPk(Connection aConn, Clasz aCriteria) throws Exception {
		Clasz result = null;
		boolean didCriteriaMatching = false;
		...
		ObjectBase.GetLeafSelectCriteria(aCriteria, whereBox); // the select criteria for this leaf clszObject, doesn't do the parent clszObject
		Record recWhere = whereBox.get(aCriteria.getTableName());
		int cntrRec = 0;
		if (recWhere != null) {
			// place in the where criteria into sql string
			StringBuffer strBuffer = new StringBuffer();
			List<Field> aryWhere = Database.GetWhereClause(aCriteria.getTableName(), recWhere, strBuffer); // convert the where record into array list
			String sqlStr = "select * from " + aCriteria.getTableName();
			...
			try {
				...
				while (rs.next()) {
					if (cntrRec > 0) {
						throw new Hinderance("Object to fetch criteria returns more then one record: '" + aCriteria.getClass().getSimpleName() + "'");
					}
					aCriteria.setObjectId(rs.getLong(aCriteria.getPkName()));
					result = aCriteria;
					cntrRec++;
					didCriteriaMatching = true;
				}
			} catch (Exception ex) {
				if (stmt != null) {
		...
		if (didCriteriaMatching && cntrRec != 1) {
		} else {
			if (ObjectBase.ParentIsNotAtClaszYet(aCriteria) && cntrRec == 0) { // still not found the record
				...
				if (parentObject != null && parentObject.getClass().equals(Clasz.class) == false) { // no parent clszObject because its an abstract class and no inherited normal class
					if (aCriteria.isPopulated() == false) {
						result = FetchObjectForPk(aConn, parentObject); // traverse the inheritance tree until we get the oid of any of the object  
					}
				}
			}
		}
	...
	public static boolean fetchObjectUpTheTree(Connection aConn, Clasz aResult, Long aParentKey) throws Exception {
		...
		if (parentObject != null && parentObject.getClass().equals(Clasz.class) == false && ObjectBase.ParentIsNotAbstract(aResult)) {
			sqlStr = "select * from " + aResult.getTableName() + ", " + aResult.getInheritanceTableName(); // got parent class, do the sql matching
			sqlStr += " where " + aResult.getTableName() + "." + aResult.getPkName() + " = " + aResult.getInheritanceTableName() + "." + aResult.getPkName();
	...
	public static void fetchObjectUpTheTree2(Connection aConn, Clasz aResult, Long aParentKey) throws Exception {
		...
		if (ObjectBase.ParentIsNotAtClaszYet(aResult) && ObjectBase.ParentIsNotAbstract(aResult)) {
			if (parentObject != null && ObjectBase.ParentIsNotAtClaszYet(aResult)) {
				sqlStr = "select * from " + aResult.getTableName() + ", " + aResult.getInheritanceTableName() + ", " + parentObject.getTableName(); // got parent class, do the sql matching
				sqlStr += " where " + aResult.getTableName() + "." + aResult.getPkName() + " = " + aResult.getInheritanceTableName() + "." + aResult.getPkName();
				sqlStr += " and " + aResult.getTableName() + "." + aResult.getPkName() + " = ?";
		...
		if (parentObject != null) {
			...
			if (cntrRec == 1 && (parentParentObject != null && ObjectBase.ParentIsNotAtClaszYet(parentObject))) {
				sqlStr = "select * from " + parentObject.getTableName() + ", " + parentObject.getInheritanceTableName(); // got parent class, do the sql matching
				sqlStr += " where " + parentObject.getTableName() + "." + parentObject.getPkName() + " = " + parentObject.getInheritanceTableName() + "." + parentObject.getPkName();
	...
	public static Clasz fetchObjectFast(Connection aConn, Clasz aCriteria, boolean aFetchAllField) throws Exception {
		...
		Map<String, Record> whereBox = new ConcurrentHashMap<>(); // each table name (string) and a record for the where fields (record)
		ObjectBase.GetLeafSelectCriteria(aCriteria, whereBox);
	...
	*/
	 ...
	 */
	public ConcurrentHashMap<String, Field> refreshClaszField() throws Exception {
		this.loadClaszField();
		return(this.claszField);
	}

	public ConcurrentHashMap<String, Field> getClaszField() throws Exception {
	...
	public ConcurrentHashMap<String, Field> loadClaszField() throws Exception {
		this.claszField.clear();
		GetClaszField(this, claszField);
		return(this.claszField);
	}
	 ...
	 * @throws Exception
	 */
	private static void GetClaszField(Clasz aClasz, ConcurrentHashMap<String, Field> aResult) throws Exception {
		try {
			if (aClasz.getRecord() != null) { // recursive clasz will not have any record to it
				...
				if (ObjectBase.ParentIsNotAtClaszYet(aClasz)) {
					...
					if (parentClasz != null) { // its possible the parent class for this clszObject is abstract and still not at Clasz
						GetClaszField(parentClasz, aResult);
					}
				}
			}
		} catch (Exception ex) {
	...
	public static int compare(Clasz aLeft, Clasz aRight) throws Exception {
		...
		for(Field eachField : left.getClaszField().values()) {
		...
		/* this is a bug, no digging is needed since above getClaszField already flatten the fields to the given pass in clasz
		// if compare result = 0 (ordering is equal) or no sort key at all, dig into non atomic fields to compare
		if (result == 0) {
			for(Field eachField : left.getClaszField().values()) {
				if (eachField.isAtomic() == false) {
					if (eachField.getSortOrder() == null || eachField.getSortOrder() == SortOrder.ASC) {
						result = eachField.compareTo(right.getField(eachField.getFieldName()));
					} else {
						Field rightField = right.getField(eachField.getFieldName());
	...
	public List<Field> getFieldListByDisplayPosition() throws Exception {
		List<Field> result = new CopyOnWriteArrayList<>(this.getClaszField().values());
	...
	private static void getAllIndexKey(List<Field> result, Clasz aClasz, String aFqFieldName) throws Exception {
		for(Field eachField : aClasz.getClaszField().values()) { // no objectid field
	...
	private static void clearAllIndexKey(Clasz aClasz) throws Exception {
		for(Field eachField : aClasz.getClaszField().values()) {
			if (eachField.isAtomic()) {
				if (eachField.isObjectKey()) {
	...
	public static void ForEachClasz(Connection aConn, Class aClass, String aSqlToGetObjId, ResultSetFetchIntf aCallback) throws Exception {
		List<Field> noSqlParam = new CopyOnWriteArrayList<>();
		ForEachClasz(aConn, aClass, aSqlToGetObjId, noSqlParam, aCallback);
	}

	public static void ForEachClasz(Connection aConn, Class aClass, String aSqlToGetObjId, List<Field> aPositionalParamValue, ResultSetFetchIntf aCallback) throws Exception {
	...
	public static FieldType GetFieldType(Connection aConn, Class aClass, String aFieldName) throws Exception {
		...
		ConcurrentHashMap<String, Field> claszFields = aClasz.getClaszField();
	...
	public CopyOnWriteArrayList<Clasz> getParentObjectByDbOfField(Connection aConn, Class aParent, String aFieldName) throws Exception {
		...
		CopyOnWriteArrayList<Clasz> result = new CopyOnWriteArrayList<>();
		String memberTableName = Clasz.GetMemberOfTableName(aParent); // get the iv_ table name
		Table memberTable = new Table(memberTableName); // create the iv_ table
		String parentPkName = CreatePkName(aParent);
	...
	public CopyOnWriteArrayList<Clasz> getAllParentOfMemberFieldBox(Connection aConn, Class aParent, String aFieldName) throws Exception {
		CannotBeInline(aParent, aFieldName);
		CopyOnWriteArrayList<Clasz> result = new CopyOnWriteArrayList<>();
		String boxMemberTableName = Clasz.GetBoxMemberTableName(aParent, aFieldName); // get the iv_ table name
	...
	public int compareForCriteria(Connection aConn, Clasz aRealClasz) throws Exception {
		int compareInt = -1;
		for(Field eachCriteria : this.getClaszField().values()) {
...
App.logDebg(Clasz.class, "Clasz.FetchPageFromTable: " + stmt);
	...
	*/
	...
	public static void FetchAll(Connection aConn, Class aClass, ResultSetFetchIntf aCallback) throws Exception {
		...
		String pkColName = Clasz.CreatePkName(aClass);
		...
		try {
			...
			while (rset.next()) {
				...
				if (aCallback != null) {
					if (aCallback.callback(aConn, fetchMember) == false) {
						break;
					}
				}
			}
	...
	*/
	...
	public static void FetchParentByBoxMember(Connection aConn, Class aClass, String aMemberName, Clasz aMemberObject, ResultSetFetchIntf aCallback) throws Exception {
		if (Clasz.class.isAssignableFrom(aClass) == false) {
		...
		String parentPkName = Clasz.CreatePkName(aClass);
		String memberTableName = GetBoxMemberTableName(aClass, aMemberName);
	...
	public static void FetchParentByMember(Connection aConn, Class aParentClass, String aMemberName, Clasz aMemberObject, ResultSetFetchIntf aCallback) throws Exception {
		...
		String parentPkName = Clasz.CreatePkName(aParentClass);
		String memberTableName = GetMemberOfTableName(aParentClass);
		String memberFieldName = CreateDbFieldName(aMemberName);
		...
		try {
			...
			while (rset.next()) {
				...
				if (aCallback != null) {
					if (aCallback.callback(aConn, fetchParent) == false) {
						break;
					}
				}
			}
